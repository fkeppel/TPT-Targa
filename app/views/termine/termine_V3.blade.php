<style>
    ...
    :root {
        ...
        /* "Helvetica Neue", Roboto, -apple-system, "Segoe UI", system-ui,
            Arial, "Noto Sans", "Liberation Sans", sans-serif; */
        --font-base: 9px;
        --font-step: 2px;
        --font-xs: var(--font-base);
        --font-s: calc(var(--font-base) + var(--font-step));
        --font-m: calc(var(--font-base) + (var(--font-step) * 2));
        --font-l: calc(var(--font-base) + (var(--font-step) * 8));
        /* Spaltenbreiten */
        --col1-ian: 90px;
        --col2-phase: 60px;
        --col3-artikel: 180px;
        ...
        --col8-ma: 150px;
        --col-term: 40px;
        --col-term-header-h: 150px;
        --col-term-header-label-w: 120px;
    }
    * {
    ...
    .table-cell_value {
        display: table-cell;
        width: var(--col-term);
        min-width: var(--col-term);
        max-width: var(--col-term);
        height: var(--row-h);
    ...
    /* =========================================================
    9. Header-Rotation / Termin-Basis
    ========================================================= */
    .termin_cell {
        width: var(--col-term);
        height: 150px;
    ...
    /* =========================================================
    10. Status / CRD / Übergabe / PM / Vertretung
    ========================================================= */
    .statusBlock {
        padding: 0;
        border-radius: var(--radius);
    ...
    .termNotNeeded {
        ...
        ----background-color: rgb(25, 157, 45);
    ...
    <div id="overlayContent">
        ...
        <form id="FormAbsagegrund">
            <select name="inpAbsagegrund" id="inpAbsagegrund" class="form-select" required>
                ...
                <option value="kein adäquates Angebot möglich">
                    {{ ServiceProvider::tl($lang, 'kein adäquates Angebot möglich') }}</option>
                ...
                <option value="Aufgrund von Patentrechten nicht möglich">
                    {{ ServiceProvider::tl($lang, 'Aufgrund von Patentrechten nicht möglich') }}</option>
                <option value="KAT Nachbestellung - Angebot nicht möglich">
                    {{ ServiceProvider::tl($lang, 'KAT Nachbestellung - Angebot nicht möglich') }}</option>
            </select>
        ...
        @foreach ($SALs as $sal)
            @if ($board == $sal->PPBoard_Id)
                <div class="boardHeader">
                    {{ ServiceProvider::tl($lang, $sal->PPBoard_Bezeichnung) }}  (V3) ({{$lang}})
                ...
                <thead>
                    <tr class="table-row-header">
                        ...
                        @foreach ($a_oberbez as $obez)
                            <th class="table-cell-header-rotate cpccolAll" style="background-clip: padding-box; font-size:var(--font-s);padding:0px;background-color:<?php $obez['bg']; ?>;" colspan="{{ $obez['colspan'] }}">
                    ...
                    <tr class="table-row-header">
                        ...
                        @if (Auth::User()->PPMitarbeiter_Gruppe == 'admin' or Auth::User()->isMaster)
                            <td class="table-cell-header-rotate" style="background-clip: padding-box;padding:0px;border-top:none;vertical-align: bottom;">
                                <div style="border-radius:0px;border-top:1px solid #003D7C;background-color:transparent;height:45px;padding:2px;">
                                    ...
                                    <select style='margin-top:5px;width:90px;padding:3px;border-radius:0px;font-size:var(--font-m);' id='selectTaetigkeit' onchange="fillSelect(this);">
                                        <option value=''></option>
                        ...
                        @foreach ($headers as $header)
                            ...
                            <td class="table-cell-header-rotate" style="padding:0px;background-clip: padding-box;">
                                <div class="termin_cell"  title="{{ ServiceProvider::tl($lang, $header->PPBoardSpalte_Bezeichnung) }}">
                                ...
                                <td class="table-cell_value termCell" data-bg data-bgval="rgb({{ isset($t['bgcolor']) ? $t['bgcolor'] : '255,0,0' }})">
                                    <div class="cellClip pad0">
                                        ...
                                        <div class="termClickArea" data-bg data-bgval="rgb({{ isset($t['bgcolor']) ? $t['bgcolor'] : '255,0,0' }})" title="{{ $title }}" onclick="ajax_getTermin({{ $pp['id'] }},{{ $t['id'] }},{{ $board }})">
                                            ...
                                            <span class="termLabel" title='{{ $orgLabel1 }}'>{{ $orgLabel1 }}</span>
                                        </div>
                                    </div>
                                </td>
                            @endforeach
