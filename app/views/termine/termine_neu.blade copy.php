...
<div id="overlay">
    <div id="overlayContent">
        <h2>Absagegrund bitte angeben:</h2>
        <form id="FormAbsagegrund">
            <select name='inpAbsagegrund' id='inpAbsagegrund' class='selectUebergabe' required style="font-size:0.9em;padding:6px;height:30px;width:300px;">
                ...
                <option >kein adäquates Angebot möglich</option>
                <option >kein Lieferant</option>
                <option >Storno LIDL</option>
                <option >Aufgrund von Patentrechten nicht möglich</option>
                <option >KAT Nachbestellung - Angebot nicht möglich</option>
            </select>
...
<div style="padding:4px; border-radius:0px; border:1px solid darkblue; min-height:100px!important;max-height:300px!important;overflow:hidden;width:calc(100% - 23px)">
    ...
    <div id="cFormHeader" style="border:none;" >
        <div class="searchTab"  style="display:grid;grid-template-columns: repeat(auto-fit, 120px 200px);grid-gap:10px; font-family:tahoma; font-size:14px;border:none;border-radius:0px;overflow:hidden;">
                ...
                <input type='checkbox' style="width:30px;margin:0px;" id="inpPPProduktpass_IsCriticalProjectReal"  name="sQry[PPProduktpass_IsCriticalProject]"  @if(@isset($kalender['SP']['PPProduktpass_IsCriticalProject']) and $kalender['SP']['PPProduktpass_IsCriticalProject'] == 1 ) checked=checked @endif value='1'/></div>
...
@if (strlen($t['label']) > 0) Label:{{{ $t['label'] }}} @endif " style="overflow:hidden;position:relative;width:30px;height:66px;background-color:rgb({{isset($t['bgcolor'])?$t['bgcolor']:'255,0,0'}});border-radius:0px;" onclick="ajax_getTermin({{$pp['id']}},{{$t['id']}},{{$board}})" >
