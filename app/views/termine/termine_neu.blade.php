...
<div id="overlay">
    <div id="overlayContent">
        ...
        <button type="submit" class="btn-primary">{{ ServiceProvider::tl($lang, 'Absenden') }}</button>
        </form>
    </div>
</div>
...
<div style="padding:4px; border-radius:0px; border:1px solid darkblue; min-height:100px!important;max-height:115px!important;overflow:hidden;width:calc(100% - 23px)">
    ...
    <div id="cFormHeader" style="border:none;" >
        <div class="searchTab"  style="display:grid;grid-template-columns: repeat(auto-fit, 120px 200px);grid-gap:10px; font-size:14px;border:none;border-radius:0px;overflow:hidden;">
                ...
                <div class='stValue'>                    
                    <select  id="inpFormPM"  name="sQry[PJMler]" >
                        <option></option>
                        @foreach ($PJMs as $id => $mx5)
                        <option title='{{ isset($mitarbeiterNamen[$id]->PPMitarbeiter_Name)?$mitarbeiterNamen[$id]->PPMitarbeiter_Name.", ".$mitarbeiterNamen[$id]->PPMitarbeiter_Vorname:"NNN";  }}'  value="{{ $mx5}}" @if(isset($kalender['SP']['PJMler']) && $kalender['SP']['PJMler']  == $mx5 ) selected @endif >{{ $mx5 }}</option>
                        @endforeach
                    </select>
                </div>
                ...
                <div class='stValue' style='border:none;padding:0px;'><input type='hidden' id="inpPPProduktpass_IsCriticalProject"  name="sQry[PPProduktpass_IsCriticalProject]"  value="0"/>
                    <input type='checkbox' style="width:30px;margin:0px;" id="inpPPProduktpass_IsCriticalProjectReal"  name="sQry[PPProduktpass_IsCriticalProject]"  @if(@isset($kalender['SP']['PPProduktpass_IsCriticalProject']) and $kalender['SP']['PPProduktpass_IsCriticalProject'] == 1 ) checked=checked @endif value='1'/>
                </div>
                <div class='stLabel'>{{ ServiceProvider::tl($lang, '#Zeilen') }}</div>
                <div class='stValue'><input  style="width:100px;" id="rows"  name="rows"  value="{{$kalender['rows']}}"/> <span style="display:inline;">{{ ServiceProvider::tl($lang, 'Gesamt')}}: {{ number_format($kalender['totalRows'], 0, ',', '.') }}</span></div>
                <div class='stValue'><button id="submitFormBtn" type="button" name='action' value="anzeigen" onclick="sbmtForm();" style="width:100%;height:30px;"><b>{{ ServiceProvider::tl($lang, 'Anzeigen') }}</b></button></div>
...
@endif
    <table class="table">
        ...
        <tr class="table-row-header">
            ...
            @endif
            <!-- Spalte 12-->
            <th class="table-cell-header-rotate cpccol9" style="background-clip: padding-box;border-bottom:none;"></th>
            @if($board == 6)
            <!-- Spalte 12-->
        ...
        <tr class="table-row">
            ...
            <?php
                ...
                if ($hasCrd) {
                    ...
                    $crdw = (int)$value['PP']->PPProduktpass_Liefertermin;      // nur wenn das wirklich KW ist!
                    $crdy = (int)$value['PP']->PPProduktpass_LieferterminJahr;  // muss ISO-week-year sein
                }
            ...
            <td class="table-cell" style="background-color: {{$bgProject}};word-wrap: break-word;">
                ...
                @else 
                        <div style="border-radius:0px; background-color:transparent;padding:0px;padding-left: 4px;">{{ strtoupper(ServiceProvider::tl($lang,$pp['InternerStatus'] ) )}}</div>
                @endif    
            ...
            <td class="table-cell"  style="text-align:center; background-color: {{$bgProject}};">
                ...
                <table class='PM'>
                    ...
                    <tr>
                        ...
                        @endif
                        <td>{{$pp['PJMler']}}</td>
                        <td><span id="VTRDISPJM{{$pp['id']}}">@if($pp['PJMlerVTR'] != '')<b style="color:orangered;">{{$pp['PJMlerVTR']}}</b> @endif</span></td>
                    </tr>
                    <tr>
                    @if($allowVTRTC)
            ...
            <?php
                ...
                //cpcDebug::cpc_debug($header->PPBoardSpalte_Id,'-ERROR_DB'); 
                //cpcDebug::cpc_debug($value['Termin'],'-ERROR_DB');
                try{
                    ...
                    cpcDebug::cpc_debug('Fehler beim Aufbau des Dasboards','-ERRORForm');
                ...
                if ($_COOKIE['TPTLanguage'] == 'EN'){
                    $t['history']   = $value['Termin'][$header->PPBoardSpalte_Id]->PPTermine_HistoryEN;
                }
                $t['log']       = $value['Log'][$header->PPBoardSpalte_Id];
                $dbV = $value['Dashboard'][$header->PPBoardSpalte_Id];
                ...
                $t['labelEN']   = $value['Termin'][$header->PPBoardSpalte_Id]->PPTermine_LabelEN;
                $t['bemerkung'] = $value['Termin'][$header->PPBoardSpalte_Id]->PPTermine_Bemerkungen;
                $t['bemerkungEN'] = $value['Termin'][$header->PPBoardSpalte_Id]->PPTermine_BemerkungenEN;
                if ($lang == 'DE'){
                    $orgLabel1 = $t['label'];
                ...
                //$t['label'] = ServiceProvider::tl($lang, $value['Termin'][$header->PPBoardSpalte_Id]->PPTermine_Label);
                ...
                //}
            ?>
            <td class="table-cell_value" style="border:1px solid #003D7C; overflow: hidden; padding:0px;vertical-align: top;background-color:rgb({{isset($t['bgcolor'])?$t['bgcolor']:'255,0,0'}})">
                @if($t['OKSTATUS'] == 'NO') 
...
@if (strlen($t['label']) > 0) Label:{{{ $t['label'] }}} @endif " style="overflow:hidden;position:relative;width:30px;height:60px;background-color:rgb({{isset($t['bgcolor'])?$t['bgcolor']:'255,0,0'}});border-radius:0px;" onclick="ajax_getTermin({{$pp['id']}},{{$t['id']}},{{$board}})" >
                    <!--div style="font-size:4px;">{{$t['ende']}}<br>{{$t['rot']}}</div -->
                    @if ($t['status'] == 'nicht benötigt')
                    <span style="font-size: 8px;">@if ($lang == 'DE')nicht benötigt @else not needed @endif</span>
                    @endif 
...
<script>
    ...
    function uebergabe (ppid, art, board){
        elem = document.getElementById('uebergabe_'+ art +'_' + ppid);
        maid = elem.value;
        if (maid != 0){
    ...
    function setzeStatus (ppid){
        ...
        if (state != 0){
            if (state == 'ABSAGE'){
                ...
                const inpPPId = document.getElementById('inpPPId');
                inpPPId.value = ppid;
    ...
    function saveFilter(board) {
        ...
        if (critPro) {
            qryFilter['PPProduktpass_IsCriticalProject'] = 1;
        } else {
            qryFilter['PPProduktpass_IsCriticalProject'] = 0;
        }
   ...
   function updateStatus(id, state, grund ='') {
    ...
    function setMA_PM_TC(id, art, maid, board) {
        ...
        function onSuccess_setMA_PM_TC(jsonResult) {
            ...
            console.log("data2:");
            console.log(data);
            var elem = document.getElementById('uebergabe_' + data.art + '_' + data.id);
            elem.value = '';
    ...
    function VertretungShow (art, ppid){
       console.log ('VertretungShow: ' + art + ' ' + ppid);
        var inp = document.getElementById ('VTRINP'+art+ppid);
        console.log('Show');
        console.log(inp);
        if (inp){
    ...
    function VTRADD (art, ppid){
        ...
        var dis = document.getElementById ('VTRDIS'+art+ +ppid);
        if (vtr){
            if (vtr.value == 0){
                dis.innerHTML = '';
            } else {
                dis.innerHTML = '' + vtr.options[vtr.selectedIndex].text;
            }
    ...
    function onSuccess_setVTR(jsonResult) {
        ...
        //elem.value = '';
        VertretungHide (data['art'], data['id']);
        alert( data['message'] );
        //window.location.reload();
        }
    function prevPage(){
        ...
        if (page -1 > 0){
            page = page -1;
            console.log("Prev page vor Call: " + page);
            elemInp.value = page;
            subm();
        } else {
    ...
    function nextPage(){
        ...
        if (page < elemPages.value){
            page = page + 1;
            console.log("Next page vor Call: " + page);
            elemInp.value = page;
            subm();
        } else {
   ...
   function _fillSelectPM_Save(  ppid, select, art ){
        var pms = [];
        if (art == 'PM'){
    ...
    function _fillSelectPM(  ppid, select, art ){
        ...
        console.log("Sortierung Neu mit MAP:");
        const sortedpms = new Map(
            Object.entries(pms).sort(([, v1], [, v2]) => v1.localeCompare(v2, "de"))
        );
        console.log(sortedpms);
        const select2 = document.getElementById("uebergabe_" + art + "_" + ppid);
        select2.options[0] = new Option("", "", true, true);
        select2.options[0].disabled = true;
        for (const [id, label] of sortedpms) {  // <- for...of, nicht for...in
            const option = document.createElement("option");
            option.value = id;
    ...
    form.addEventListener("submit", (e) => {
      e.preventDefault(); // verhindert echtes Absenden (Seitenreload)
      // Hier kannst du deine Formular-Logik einbauen (z. B. Ajax)
