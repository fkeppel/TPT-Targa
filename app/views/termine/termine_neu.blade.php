<style>
    ...
    :root{
      ...
      --c-border:lightgray;
      --radius:0px;
      --row-h:60px;     /* Dashboard Zeilenhöhe */
      --filter-h:32px;  /* Suchfeld-Kacheln (Label+Value) */
      /* bessere Kleinschrift */
      --font-ui: "Helvetica Neue", Roboto, -apple-system, "Segoe UI", system-ui,
                  Arial, "Noto Sans", "Liberation Sans", sans-serif;
      --col1-ian:        90px;   /* Sticky IAN */
      --col2-phase:      60px;   /* Phase/Ausmust. */
      --col3-artikel:    180px;  /* Artikel */
      --col4-status:     120px;  /* Interner/Lidl Status */
      --col5-ppstatus:   60px;   /* PP Status / Import / Lidl-Date */
      --col6-crd:        90px;   /* CRD/DDP */
      --col7-uebergabe:  100px;  /* Übergabe an (nur Admin/Master) */
      --col8-ma:         120px;  /* Mitarbeiter */
    }
    /* Spaltenbreiten der ersten 8 Spalten */
    .col1{ width:var(--col1-ian);        min-width:var(--col1-ian);        max-width:var(--col1-ian); }
    .col2{ width:var(--col2-phase);      min-width:var(--col2-phase);      max-width:var(--col2-phase); }
    .col3{ width:var(--col3-artikel);    min-width:var(--col3-artikel);    max-width:var(--col3-artikel); }
    .col4{ width:var(--col4-status);     min-width:var(--col4-status);     max-width:var(--col4-status); }
    .col5{ width:var(--col5-ppstatus);   min-width:var(--col5-ppstatus);   max-width:var(--col5-ppstatus); }
    .col6{ width:var(--col6-crd);        min-width:var(--col6-crd);        max-width:var(--col6-crd); }
    .col7{ width:var(--col7-uebergabe);  min-width:var(--col7-uebergabe);  max-width:var(--col7-uebergabe); }
    .col8{ width:var(--col8-ma);         min-width:var(--col8-ma);         max-width:var(--col8-ma); }
    ...
    *{ box-sizing:border-box; }
    ...
    .linkBrand{ color:var(--c-brand); text-decoration:none; }
    /* =========================================================
    Overlay Absagegrund
    ========================================================= */
    #overlay{
    ...
    #overlayContent h2{ margin:0 0 10px 0; }
    #overlayContent form{
      display:flex;
    ...
    Header
    ========================================================= */
    .boardHeaderWrap{ text-align:left; margin-top:15px; }
    .boardHeader{
      position:relative;
      padding:10px;
      font-weight:bold;
    ...
    .boardHeader--archive{ height:auto; }
    .boardHeaderLogo{
      position:absolute;
      top:8px;
      right:2px;
    ...
    .formGetTermine{ color:var(--c-brand-2); margin:5px; }
    .cFormHeader{ border:none; max-height:300px !important; }
    .searchGrid{
      display:grid;
    ...
    .stValue--plain{ border:none; padding:0; }
    ...
    .checkboxTiny{ width:30px; margin:0; }
    /* #Zeilen */
    .rowsInput{ width:70px; }
    .inlineNote{
      white-space:nowrap;
      font-size:11px;
      line-height:var(--filter-h);
    ...
    .schedule--std{ width:calc(100% - 15px); height:calc(100% - 285px); }
    .schedule--ff{ width:calc(100vw - 20px); height:calc(100vh - 270px); }
    .table{
      margin-left:10px;
      display:table;
      text-align:left;
    ...
    .bold{ font-weight:bold; padding-left:4px; }
    .termin_cell{
      width:18px;
      height:20px;
      border:none;
    ...
    .rotate--usa{ font-weight:bold; }
    /* =========================================================
    TBODY building blocks
    ========================================================= */
    .cellSticky{ width:80px; padding:0; }
    .cellStickyInner{
    ...
    .ianMain{ font-size:12px; font-weight:bold; }
    .parentChildLine{ line-height:1.1; background-color: transparent;}
    .altLineWrap{
        background-color: transparent;
    }
    .altBtn{
    ...
    /* Status / Selects */
    .statusBlock{ border-radius:var(--radius); background-color:transparent; padding:0; }
    .statusBlockPad{ padding-left:4px; }
    .setStatusSelect{
        font-size:.7rem;
        width:86px;
    ...
    .setStatusBtn{ width:92px; }
    /* CRD */
    .crdTable{ margin-left:0; font-size:10px; table-layout:fixed; }
    .crdSmall{ font-size:8px; }
    /* Übergabe */
    .uebergabeCell{ padding:0; }
    .selectUebergabe{ width:90px; height:26px; border:1px solid gray; font-size:.7rem; padding:1px; }
    .uebergabeBtn{ width:90px; margin-top:8px; }
    /* PM/PJM/TC Tabelle */
    .pmTable{
      width:100%;
      table-layout:fixed;
    ...
    /* Vertretung selects */
    .vtrSelect{ font-size:.7rem; padding:5px; width:100%; }
    .VTRINPHide{ position:relative; display:none; height:30px; border-radius:var(--radius); }
    .VTRINPShow {
        position:absolute;
        left:0;
        bottom:-32px;              /* unterhalb der Zelle anzeigen */
        width:120px;               /* sinnvoll größer */
        height:36px;
        border-radius:var(--radius);
        background:#fff;
        border:1px solid var(--c-brand-2);
    ...
    .divSelectTaetHide{ display:none; }
    .divSelectTaetShow{ display:block; border-radius:var(--radius); width:100%; height:73px; padding:0; }
    /* =========================================================
    Termin-Zelle
    ========================================================= */
    .termCell{
    ...
    .termDeadlineTextWhite{ color:#fff; }
    .termClickArea{
        position:absolute;
        top:12px;
        left:0;
        ...
        overflow:hidden;
        z-index:2;
        white-space:normal; /* nur hier Umbruch */
    }
    .termLabel{
        flex:1 1 auto;
        display:block;
    ...
    .cellClip{
        ...
        overflow:visible;      /* Overlay darf raus */
    }
    /* Padding-Varianten */
    .cellClip.pad3{ padding:3px; }
    .cellClip.pad8{ padding:8px; }
    .cellClip.pad0{ padding:0; }
    ...
    .cellStickyInner{ height:100%; border:none;}
    ...
    .VTRINPShow{
      ...
      box-shadow:0 4px 10px rgba(0,0,0,.25);
      padding:4px;
      z-index: 6000;         /* innerhalb der Zeile ganz oben */
    }
    ...
    .VTRINPShow{
      position:absolute;
      left:0;
      top:0;                 /* wir setzen top per JS dynamisch */
    ...
    .boardPage{
        ...
        height: calc(100vh - 64px);        /* ggf. anpassen */
        display: flex;
        flex-direction: column;
      ...
      .schedule{
        flex: 1 1 auto;
        min-height: 0;                      /* wichtig, sonst overflow-bugs */
        overflow: auto;
        width: calc(100% - 15px);           /* dein bisheriger std-Wert */
      }
...
<div class="boardPage">
    ...
    <div class="filterBox" id="filterBox">
      {{ Form::open(array('url' => 'termine','class'=>'formGetTermine', 'id'=>'formgetTermine','onkeypress' => 'submitFormX(event);')) }}
      ...
      <div id="cFormHeader" class="cFormHeader">
        <div class="searchTab searchGrid">
          ...
          <div class="stValue">
            <select id="inpFormPM" name="sQry[PMler]">
              ...
              @foreach ($PMs as $id => $mx5)
                <option title='{{ isset($mitarbeiterNamen[$id]->PPMitarbeiter_Name)?$mitarbeiterNamen[$id]->PPMitarbeiter_Name.", ".$mitarbeiterNamen[$id]->PPMitarbeiter_Vorname:"NNN";  }}'
                        value="{{ $mx5}}" @if(isset($kalender['SP']['PMler']) && $kalender['SP']['PMler']  == $mx5 ) selected @endif>{{ $mx5 }}</option>
              @endforeach
            </select>
          </div>
          ...
          <div class="stValue">
            <select id="inpFormPJM" name="sQry[PJMler]">
              ...
              @foreach ($PJMs as $id => $mx5)
                <option title='{{ isset($mitarbeiterNamen[$id]->PPMitarbeiter_Name)?$mitarbeiterNamen[$id]->PPMitarbeiter_Name.", ".$mitarbeiterNamen[$id]->PPMitarbeiter_Vorname:"NNN";  }}'
                        value="{{ $mx5}}" @if(isset($kalender['SP']['PJMler']) && $kalender['SP']['PJMler']  == $mx5 ) selected @endif>{{ $mx5 }}</option>
              @endforeach
            </select>
          </div>
          ...
          <div class="stValue">
            <select id="inpFormTC" name="sQry[TCler]">
              ...
              @foreach ($TCs as $id => $mx5)
                <option title='{{ isset($mitarbeiterNamen[$id]->PPMitarbeiter_Name)?$mitarbeiterNamen[$id]->PPMitarbeiter_Name.", ".$mitarbeiterNamen[$id]->PPMitarbeiter_Vorname:"NNN";  }}'
                        value="{{ $mx5}}" @if(isset($kalender['SP']['TCler']) && $kalender['SP']['TCler']  == $mx5 ) selected @endif>{{ $mx5 }}</option>
              @endforeach
            </select>
          </div>
          ...
          <div class="stValue">
            <input id="inpFormPPProduktpass_RevisionDatum" name="sQry[PPProduktpass_RevisionDatum]" placeholder="Import: 2024-10-24"
                  @if (isset($kalender['SP'])) value="{{isset($kalender['SP']['PPProduktpass_RevisionDatum'])?$kalender['SP']['PPProduktpass_RevisionDatum']:''}}" @endif/>
          </div>
          ...
          <div class="stValue stValue--pager">
            ...
            <span class="pagerMeta">{{ ServiceProvider::tl($lang,'von') }} {{ ceil($kalender['totalRows']/$kalender['rows']) }}</span>
          ...
          <div class="stValue stValue--plain">
            ...
            <input type="checkbox" class="checkboxTiny" id="inpPPProduktpass_IsCriticalProjectReal" name="sQry[PPProduktpass_IsCriticalProject]"
                  @if(@isset($kalender['SP']['PPProduktpass_IsCriticalProject']) and $kalender['SP']['PPProduktpass_IsCriticalProject'] == 1 ) checked @endif value="1"/>
          </div>
          <div class="stLabel">{{ ServiceProvider::tl($lang, '#Zeilen') }}</div>
          <div class="stValue">
            <input class="rowsInput" id="rows" name="rows" value="{{$kalender['rows']}}"/>
            <span class="inlineNote">{{ ServiceProvider::tl($lang, 'Gesamt')}}: {{$kalender['totalRows']}}&nbsp;&nbsp;</span>
          </div>
          ...
          <div class="stValue"><button id="submitFormBtn" class="actionBtn" type="button" onclick="saveFilter({{$board}});"><b>{{ ServiceProvider::tl($lang, 'Filter speichern *')}}</b></button></div>
          <div class="stValue"><button id="submitFormBtn" class="actionBtn" type="button" onclick="getFilter({{$board}});"><b>{{ ServiceProvider::tl($lang,'Filter laden *')}}</b></button></div>
        ...
        <thead>
          <tr class="table-row-header">
            ...
            <?php
              ...
              $temp_h    = "X";
              $colndx    = 0;
              $colors = array("white","lightgray","white","lightgray","white","lightgray","white","lightgray","white",
          ...
          <tr class="table-row-header">
            ...
            <td class="table-cell-header-rotate cpcStickycol" style="background-clip: padding-box;padding:0px;border-top:none;vertical-align: bottom;">
              <div style="border-radius:0px;background-color:transparent;height:45px;padding:2px;border:1px solid darkgray;border-top:1px solid #003D7C;">{{ServiceProvider::tl($lang,'IAN')}}</div>
            </td>
            <!-- Spalte 1 -->
            <td class="table-cell-header-rotate bold" style="background-clip: padding-box;padding:0px;border-top:none;vertical-align: bottom;">
              <div style="border-radius:0px;border-top:1px solid #003D7C;background-color:transparent;height:45px;padding:2px;">{{ServiceProvider::tl($lang,'Phase')}} / {{ServiceProvider::tl($lang,'Ausmust.')}}</div>
            </td>
            <!-- Spalte 6 -->
            <td class="table-cell-header-rotate" style="background-clip: padding-box;padding:0px;border-top:none;vertical-align: bottom;">
              <div style="border-radius:0px;border-top:1px solid #003D7C;background-color:transparent;height:45px;padding:2px;">{{ServiceProvider::tl($lang,'Artikel')}}</div>
            </td>
            <!-- Spalte 2 -->
            <td class="table-cell-header-rotate bold" style="background-clip: padding-box;padding:0px;border-top:none;vertical-align: bottom;">
              <div style="border-radius:0px;border-top:1px solid #003D7C;background-color:transparent;height:45px;padding:2px;">{{ServiceProvider::tl($lang,'Interner Status')}}<br>{{ServiceProvider::tl($lang,'Lidl Status')}}</div>
            </td>
            <td class="table-cell-header-rotate" style="background-clip: padding-box;padding:0px;border-top:none;vertical-align: bottom;">
              <div style="border-radius:0px;border-top:1px solid #003D7C;background-color:transparent;height:45px;padding:2px;">{{ServiceProvider::tl($lang,'PP Status')}}<br>I-{{ServiceProvider::tl($lang,'Datum')}}<br>L-{{ServiceProvider::tl($lang,'Datum')}}</div>
            </td>
            <td class="table-cell-header-rotate" style="background-clip: padding-box;padding:0px;border-top:none;vertical-align: bottom;">
              <div style="border-radius:0px;border-top:1px solid #003D7C;background-color:transparent;height:45px;padding:2px;">CRD<br>DDP</div>
            </td>
            @if(Auth::User()->PPMitarbeiter_Gruppe == 'admin' or  Auth::User()->isMaster)
              <td class="table-cell-header-rotate" style="background-clip: padding-box;padding:0px;border-top:none;vertical-align: bottom;">
            ...
            @endif
            <td class="table-cell-header-rotate" style="background-clip: padding-box;padding:0px;border-top:none;vertical-align: bottom;">
              <div style="border-radius:0px;border-top:1px solid #003D7C;background-color:transparent;height:45px;padding:2px;">{{ServiceProvider::tl($lang,'Mitarbeiter')}}</div>
            </td>
            <?php $i = 0; ?>
            @foreach ($headers as $header)
              <?php $i++; ?>
              <td class="table-cell-header-rotate" style="padding:0px;background-clip: padding-box;" >
                <div class="termin_cell" style="height:150px; overflow: hidden;width:30px;" title="{{  ServiceProvider::tl($lang,$header->PPBoardSpalte_Bezeichnung) }}">
                  ...
                  <div class="rotate" style="overflow: hidden;width: 110px; @if($header->PPBoardSpalte_IsUSA) font-weight:bold; @endif">
                    {{substr(ServiceProvider::tl($lang,$header->PPBoardSpalte_Bezeichnung),0,20)}}
                  </div>
                </div>
              </td>
            @endforeach
              ...
              @foreach ($headers as $header)
                ...
                @if ($first)
                  <?php
                    ...
                    if (strlen($alternativArtikel) > 80) { $pp['artikel'] .= "+"; }
                    ...
                    $crd->setTime(0,0,0);
                    $hasCrd = isset($value['PP']->PPProduktpass_CRDJahr)
                            && !is_null($value['PP']->PPProduktpass_CRDJahr)
                            && (int)$value['PP']->PPProduktpass_CRDJahr !== 0;
                    if ($hasCrd) {
                    ...
                    if (!$hasCrd) { $crd->sub(new DateInterval('P9W')); }
                    ...
                    if ($value['PP']->PPProduktpass_SimNeu == -1){ $simInitColor = 'lightgreen'; }
                    if (count($fails) > 0){ $simInitColor = '#ffcccc'; }
                  ?>
                  <!-- ===============================
                      TBODY: ab hier Zellen mit .cellClip
                  ================================ -->
                  <td class="table-cell cpcStickycol cellSticky col1" data-bg data-bgval="{{$bgCritProject}}">
                      ...
                      <div class="cellStickyInner">
                        <a href="/show/{{$pp['id']}}" target="_blank" class="linkBlack">
                          <div class="ianBox">
                            ...
                            @else
                              <span class="ianMain" data-color data-colorval="{{ $pp['color_isRFQ'] }}"><b>{{$pp['ian']}}</b> {{ $isFirstRev }}</span>
                            @endif
                          </div>
                        </a>
                        <br>
                        ...
                        <div class="altLineWrap">
                          @if (strlen($pp['AltIAN']) > 0)
                            @if ($pp['IsValid_OldIAN'])
                              <a href="/showAlt/{{$pp['id']}}" target="_blank" class="linkBrand"><b>{{$pp['AltIAN']}}_{{$pp['AltCharge']}}</b></a>
                            @else
                  ...
                  <td class="table-cell cellWrapBreak col2" data-bg data-bgval="{{$bgProject}}">
                    <div class="cellClip pad3">
                      {{$pp['Musterung']}}<br>
                      {{-- <a href="/showAlt/{{$value['PP']->PPProduktpass_Id}}/ALt" target="_blank" class="noDeco">
                        <div class="altBtn">Alt</div>
                      </a> --}}
                  ...
                  <td class="table-cell cellWrapBreak col3" data-bg data-bgval="{{$bgProject}}" data-color data-colorval="{{$colArtikelBez}}" title="{{$value['PP']->PPProduktpass_Artikelbezeichnung}}">
                    <div class="cellClip pad3">
                      {{$pp['artikel']}}
                    </div>
                  </td>
                  <td class="table-cell cellWrapBreak col4" data-bg data-bgval="{{$bgProject}}">
                    <div class="cellClip pad3">
                      @if (ViewController::UserHasRole(Auth::User()->PPMitarbeiter_Id, 'STATUS'))
                        <div class="statusBlock">
                          <select id="setzeStatus_{{$pp['id']}}" class="setStatusSelect">
                            ...
                            <option @if($pp['InternerStatus']=='PLAN') selected @endif value="PLAN">{{ ServiceProvider::tl($lang, 'PLAN') }}</option>
                            <option @if($pp['InternerStatus']=='FIX') selected @endif value="FIX">{{ ServiceProvider::tl($lang, 'FIX') }}</option>
                            <option @if($pp['InternerStatus']=='GELIEFERT') selected @endif value="GELIEFERT">{{ ServiceProvider::tl($lang, 'GELIEFERT') }}</option>
                            <option @if($pp['InternerStatus']=='ABSAGE') selected @endif value="ABSAGE">{{ ServiceProvider::tl($lang, 'ABSAGE') }}</option>
                          </select>
                          <button class="setStatusBtn" type="button" onclick="setzeStatus({{$pp['id']}});">{{ ServiceProvider::tl($lang, 'setzen') }}</button>
                        </div>
                      @else
                        <div class="statusBlock statusBlockPad">{{ strtoupper(ServiceProvider::tl($lang,$pp['InternerStatus'])) }}</div>
                      @endif
                      <div class="statusBlock statusBlockPad">{{$pp['statusDoc']}}</div>
                    </div>
                  </td>
                  <!-- Spalte 5 -->
                  <td class="table-cell col5" data-bg data-bgval="{{$bgProject}}">
                    <div class="cellClip pad3">
                      <b>{{$pp['PPstatus']}}</b><br>{{$pp['DatumImport']}}<br>{{$pp['LidlUpdateDate']}}
                    </div>
                  </td>
                  <!-- CRD/DDP -->
                  <td class="table-cell col6" data-bg data-bgval="{{$bgProject}}">
                    <div class="cellClip pad3">
                      <table class="crdTable">
                        <tr>
                          <td data-color data-colorval="{{ $styleCRD }}"><b>CRD:</b></td>
                          <td><b>{{$crd->format('W') . '/' . substr($crd->format('o'), 2)}}</b></td>
                        </tr>
                        <tr>
                          <td @if($pp['ddpltw'] > 54) data-color data-colorval="red" @endif>DDP:</td>
                  ...
                  @if(Auth::User()->PPMitarbeiter_Gruppe == 'admin' or Auth::User()->isMaster)
                    <td class="table-cell uebergabeCell col7" data-bg data-bgval="{{$bgProject}}">
                      <div class="cellClip pad3">
                        ...
                        @endif
                        @if (Auth::User()->PPMitarbeiter_Taetigkeit == 'PJM' and Auth::User()->isMaster)
                  ...
                  @endif
                  <!-- Mitarbeiter -->
                  <td class="table-cell u-center col8" data-bg data-bgval="{{$bgProject}}">
                    <div class="cellClip pad3">
                      <?php
                        $user = Auth::user();
                        ...
                        $allowVTRPM = false;
                        $allowVTRPJM = false;
                        if (isset($pp['PMler']) and $kuerzel == $pp['PMler']) $allowVTRPM = true;
                        if (isset($pp['PJMler']) and $kuerzel == $pp['PJMler']) $allowVTRPJM = true;
                        if (isset($pp['TCler']) and $kuerzel == $pp['TCler']) $allowVTRTC = true;
                        if ($taetigkeit == 'PM' and ($isAdmin or $isMaster)) $allowVTRPM = true;
                        if ($taetigkeit == 'PJM' and ($isAdmin or $isMaster)) $allowVTRPJM = true;
                        if ($taetigkeit == 'TC' and ($isAdmin or $isMaster)) $allowVTRTC = true;
                      ...
                      <table class="pmTable">
                        <tr>
                          @if($allowVTRPM)
                            <td class="pointer greenText" onclick="VertretungShow('PM', {{$pp['id']}} );" title='{{ ServiceProvider::tl($lang, 'Vertreter PM setzen') }}'><b>PM</b></td>
                          @else
                            <td><b>PM</b></td>
                          @endif
                          <td>{{$pp['PMler']}}</td>
                          <td><span id="VTRDISPM{{$pp['id']}}">@if($pp['PMlerVTR'] != '')<b class="orangered">{{$pp['PMlerVTR']}}</b>@endif</span></td>
                        </tr>
                        <tr>
                          @if($allowVTRPJM)
                            <td class="pointer greenText" onclick="VertretungShow('PJM', {{$pp['id']}} );" title='{{ ServiceProvider::tl($lang, 'Vertreter PJM setzen') }}'><b>PJM</b></td>
                          @else
                            <td><b>PJM</b></td>
                          @endif
                          <td>{{$pp['PJMler']}}</td>
                          <td><span id="VTRDISPJM{{$pp['id']}}">@if($pp['PJMlerVTR'] != '')<b class="orangered">{{$pp['PJMlerVTR']}}</b>@endif</span></td>
                        </tr>
                        <tr>
                          @if($allowVTRTC)
                            <td class="pointer greenText" onclick="VertretungShow('TC', {{$pp['id']}} );" title='{{ ServiceProvider::tl($lang, 'Vertreter TC setzen') }}'><b>TC</b></td>
                          @else
                            <td><b>TC</b></td>
                          @endif
                          <td>{{$pp['TCler']}}</td>
                          <td><span id="VTRDISTC{{$pp['id']}}">@if($pp['TClerVTR'] != '')<b class="orangered">{{$pp['TClerVTR']}}</b>@endif</span></td>
                        </tr>
                      </table>
                      <div id="VTRINPPM{{$pp['id']}}" class="VTRINPHide">
                        <select id="vertretungPM{{$pp['id']}}" class="vtrSelect" onchange="VTRADD('PM', {{$pp['id']}});">
                          ...
                          @foreach ($PMs as $vid => $vtr)
                            <option title='{{ isset($mitarbeiterNamen[$vid]->PPMitarbeiter_Name)?$mitarbeiterNamen[$vid]->PPMitarbeiter_Name.", ".$mitarbeiterNamen[$vid]->PPMitarbeiter_Vorname:"NNN";  }}'
                                    @if($pp['PMlerVTR'] == $vtr) selected @endif value="{{$vid}}">{{ $vtr }}</option>
                          @endforeach
                        </select>
                      </div>
                      <div id="VTRINPPJM{{$pp['id']}}" class="VTRINPHide">
                        <select id="vertretungPJM{{$pp['id']}}" class="vtrSelect" onchange="VTRADD('PJM', {{$pp['id']}});">
                          ...
                          @foreach ($PJMs as $vid => $vtr)
                            <option title='{{ isset($mitarbeiterNamen[$vid]->PPMitarbeiter_Name)?$mitarbeiterNamen[$vid]->PPMitarbeiter_Name.", ".$mitarbeiterNamen[$vid]->PPMitarbeiter_Vorname:"NNN";  }}'
                                    @if($pp['PJMlerVTR'] == $vtr) selected @endif value="{{$vid}}">{{ $vtr }}</option>
                          @endforeach
                        </select>
                      </div>
                      <div id="VTRINPTC{{$pp['id']}}" class="VTRINPHide">
                        <select id="vertretungTC{{$pp['id']}}" class="vtrSelect" onchange="VTRADD('TC', {{$pp['id']}});">
                          ...
                          @foreach ($TCs as $vid => $vtr)
                            <option title='{{ isset($mitarbeiterNamen[$vid]->PPMitarbeiter_Name)?$mitarbeiterNamen[$vid]->PPMitarbeiter_Name.", ".$mitarbeiterNamen[$vid]->PPMitarbeiter_Vorname:"NNN";  }}'
                                    @if($pp['TClerVTR'] == $vtr) selected @endif value="{{$vid}}">{{ $vtr }}</option>
                          @endforeach
                        </select>
                      </div>
                    </div>
                ...
                @endif {{-- first --}}
            <?php
                /* ======= DEIN TERMIN-PHP (unverändert) ======= */
                $EndeW2LT = isset($po['w2fob']) ? $po['w2fob'] : 0;
                $dto = new DateTime();
                if (isset($pp['lty'], $pp['ltw'])) {
                    $dto->setISODate($pp['lty'], $pp['ltw'], 5);
                }
                $spaltenId = $header->PPBoardSpalte_Id;
                $term = (isset($value['Termin'][$spaltenId]) && is_object($value['Termin'][$spaltenId]))
                    ? $value['Termin'][$spaltenId]
                    : null;
                if ($term) { $t['id'] = $term->PPTermine_Id; }
                else {
                    $t['id'] = 0;
                    cpcDebug::cpc_debug('Fehler beim Aufbau des Dashboards', '@ERROR');
                    if (isset($value['PP']->PPProduktpass_Id)) { cpcDebug::cpc_debug($value['PP']->PPProduktpass_Id, '@ERROR'); }
                }
                $t['bgcolor']   = $term ? $term->PPStati_Background : '255,0,0';
                $t['bemerkung'] = $term ? $term->PPTermine_Bemerkungen : '';
                $t['terminart'] = $header->PPBoardSpalte_Bezeichnung;
                $t['status']    = $term ? $term->PPTermine_Status : '';
                $t['ma']        = $term ? $term->PPTermine_MAZustaendigkeit : '';
                $t['history']   = $term ? $term->PPTermine_History : '';
                $t['log']       = isset($value['Log'][$spaltenId]) ? $value['Log'][$spaltenId] : null;
                $dbV            = isset($value['Dashboard'][$spaltenId]) ? $value['Dashboard'][$spaltenId] : array();
                if ($lang === 'EN' && $term) { $t['history'] = $term->PPTermine_HistoryEN; }
                ...
                if ($term && isset($term->PPTermine_ManSollDate) && $term->PPTermine_ManSollDate != '0000-00-00 00:00:00') {
                    $thisFriday = new DateTime();
                    $thisFriday->setISODate($thisFriday->format('Y'), $thisFriday->format('W'), 5);
                    try {
                    $manSollDate = new DateTime($term->PPTermine_ManSollDate);
                    $diffManSoll = $thisFriday->diff($manSollDate);
                    $w2ManSoll   = floor(((int)$diffManSoll->format('%R%a'))/7);
                    $manSoll = isset($term->PPTermine_ManSoll) ? (int)$term->PPTermine_ManSoll : 0;
                    $t['rot']    = -1 * $manSoll - 10;
                    $t['orange'] = $t['rot'] + 2;
                    $t['IsMileStone'] = true;
                    } catch (Exception $e) {}
                ...
                $t['label']      = $term ? $term->PPTermine_Label : '';
                $t['labelEN']    = $term ? $term->PPTermine_LabelEN : '';
                $t['bemerkungEN']= $term ? $term->PPTermine_BemerkungenEN : '';
                if ($lang == 'DE'){
                    $orgLabel1 = $t['label'];
                ...
                try{
                    ...
                    $dayOfWeek = (int)$datenow->format('w'); if ($dayOfWeek > 5) $dayOfWeek = 5;
                    $datenow->add(new DateInterval('P'.(5-$dayOfWeek).'D'));
                    $crd2  = new DateTime();
                    $crdw2 = (int)$value['PP']->PPProduktpass_CRDWoche;
                    $crdy2 = (int)$value['PP']->PPProduktpass_CRDJahr;
                    if ($crdy2 == 0){
                    $crdw2 = (int)$value['PP']->PPProduktpass_Liefertermin;
                    $crdy2 = (int)$value['PP']->PPProduktpass_LieferterminJahr;
                    }
                    $crd2->setISODate($crdy2, $crdw2, 5);
                    if ($crdy2 == 0){ $crd2->sub(new DateInterval('P10W')); }
                    $stdPeriode = (int)($t['rot'] + 10);
                    if ($stdPeriode < 0) $stdPeriode *= -1;
                    $startDateNeu = clone $crd2;
                    $stdPeriodeInterval = new DateInterval('P'.$stdPeriode.'W');
                    if (($t['rot'] + 10) < 0) $startDateNeu->sub($stdPeriodeInterval);
                    else $startDateNeu->add($stdPeriodeInterval);
                    $dayOfWeek = (int)$startDateNeu->format('w'); if ($dayOfWeek > 5) $dayOfWeek = 5;
                    $startDateNeu->add(new DateInterval('P'.(5-$dayOfWeek).'D'));
                    $rwi = $datenow->diff($startDateNeu);
                    $rw  = ((int)$rwi->format("%a"))/7;
                    if ($datenow > $startDateNeu) $rw *= -1;
                    $w2startNeu = round($rw,0);
                    if ($term && $term->PPTermine_ManSollDate != '0000-00-00 00:00:00'){
                    $startDateNeu = new DateTime($term->PPTermine_ManSollDate);
                    $dayOfWeek = (int)$startDateNeu->format('w'); if ($dayOfWeek > 5) $dayOfWeek = 5;
                    $startDateNeu->add(new DateInterval('P'.(5-$dayOfWeek).'D'));
                    $rwi = $datenow->diff($startDateNeu);
                    ...
                    if ($w2startNeu == -0) $w2startNeu = 0;
                    if ($w2startNeu > 2) $bgNeu ="240,240,240";
                    if ($w2startNeu <= 2 && $w2startNeu >= 0) $bgNeu ="255,255,153";
                    if ($w2startNeu < 0) $bgNeu ="255,199,206";
                } catch (Exception $ex) {
                    $msg = $ex->getMessage();
                }
                ...
                if (!isset($t['SOLLY'])) $t['SOLLY'] = 0;
                if (!isset($t['SOLLW'])) $t['SOLLW'] = 0;
                if ($t['OKSTATUS'] == 'YES' && $t['WERLSET'] == 'YES'){
                    $t['WERLY'] = (int)$t['WERLY'] - 2000;
                    if ($t['SOLLY'] == $t['WERLY']){
                    $t['WERL'] = (int)$t['SOLLW'] - (int)$t['WERLW'];
                    } else {
                    $suby = -52;
                    if ($t['SOLLY'] > $t['WERLY']) $suby = +52;
                    $t['WERL'] = (int)$t['SOLLW'] + $suby - (int)$t['WERLW'];
                    }
                }
                $newBG = $bgNeu;
                if ($t['status'] == 'nicht benötigt'){ $newBG = '255,199,206'; }
                $failType = 'transparent';
                if (isset($mplanOK) && !$mplanOK && isset($mplanFails, $t['id'], $mplanFails[$t['id']]['stats'])){
                    $msfail = $mplanFails[$t['id']]['stats'];
                    if (isset($msfail['budget'])) $failType = 'orange';
                    if (isset($msfail['rule']))   $failType = 'red';
                }
                $deadlineTitleNo = ($w2start != $w2start_undef)
                    ? "Deadline (manuell): {$w2startNeu}W \nDeadline (berechnet): ".((int)$po['w2fob'] + (int)$t['rot'])."W"
                    : "Deadline (berechnet): ".((int)$po['w2fob'] + (int)$t['rot'])."W";
                $deadlineTitleYes = ($w2start != $w2start_undef)
                    ? "Deadline (manuell): {$w2startNeu}W \nDeadline (berechnet): ".((int)$pp['w2crd'] + (int)$t['rot'] + 9)."W"
                    : "Deadline (berechnet): ".((int)$pp['w2crd'] + (int)$t['rot'] + 9)."W";
            ?>
                    ...
                    @else
                      @if ($t['WERL'] < 0 )
                        <div class="termDeadlineBar" data-bg data-bgval="@if ($t['status'] != 'nicht benötigt') rgb(255,199,206) @else rgb(25,157,45) @endif" title="{{$deadlineTitleYes}}">
                          <span class="termDeadlineText" data-color data-colorval="darkblue">@if ($t['status'] != 'nicht benötigt'){{ $t['WERL'] }}@endif</span>
                        </div>
                      @else
                        <div class="termDeadlineBar" data-bg data-bgval="rgb(25,157,45)" title="{{$deadlineTitleYes}}">
                          <span class="termDeadlineText termDeadlineTextWhite">@if ($t['status'] != 'nicht benötigt'){{ $t['WERL'] }}@endif</span>
                        </div>
                      @endif
                    @endif
                    <div class="termClickArea"
                        data-bg data-bgval="rgb({{isset($t['bgcolor'])?$t['bgcolor']:'255,0,0'}})"
                        title="{{$dbV['Art']}}
                              CRD:{{$dbV['CRDDate']}}
                              W2Soll:{{$dbV['W2CRD']}}
                              Soll:{{$dbV['SollDate']}}
                              ...
                              @if (strlen($t['label']) > 0) Label:{{{ $t['label'] }}} @endif"
                              onclick="ajax_getTermin({{$pp['id']}},{{$t['id']}},{{$board}})">
                              @if ($t['status'] == 'nicht benötigt')
                                <span class="termNotNeeded">@if ($lang == 'DE')nicht benötigt @else not needed @endif</span>
                              @endif
...
@else
<div class="tahoma" style="padding:20px;"><h1>Keine Daten gefunden!</h1></div>
@endif
<script>
    /* =========================================================
    ...
    function onSuccess_saveFilter(jsonResult) { alert('Filter gespeichert!'); }
    ...
    function ajax_getTermin(ppid, tid, board) {
      window.open("/getTerminFromId/" + ppid + "/" + tid + "/" + board, "Termindetails", "toolbar=no, scrollbars=no, resizable=no, top=10, left=10, width=1910, height=1160");
    }
    ...
    document.addEventListener('keydown', function(e){
      if (e.key === 'Escape' || e.key === 'Esc'){
        document.querySelectorAll('.VTRINPShow').forEach(function(el){
          VertretungHide(el.id.replace('VTRINP', '').slice(0, -3), el.id.replace('VTRINP', '').slice(-3));
