...
<table class="terminHist" style="border-collapse:collapse;font-size:12px;font-family:tahoma;">
    <tr>
        ...
        <td class="thHeader2" style="width:40px;">g.v.</td>
        <td class="thHeader2" style="width:120px;">erledigt am</td>
        <td class="thHeader2" style="width:400px;">Bemerkung</td>
    ...
    <tr class="thRow {{$class}}" name="{{$class}}" >
        ...
        <td class="thCell" @if((!is_null($log->PPTermineChanges_DoneAt)) and (strlen($log->PPTermineChanges_DoneAt) >= 10)) style="background-color:lime;" @endif>
            @if(strlen($log->PPTermineChanges_DoneAt) < 10)
                @if ($log->PPTermineChanges_IsActive == 1) 
                    @if($log->PPTermineChanges_oldStatus == $log->PPTermineChanges_newStatus and $statusInfo == 'Unteraufgabe')
                        @if (isset($mitarbeiterliste[$log->PPTermineChanges_Receiver]) and Auth::User()->PPMitarbeiter_Kuerzel == $mitarbeiterliste[$log->PPTermineChanges_Receiver])
                            <button id="btnErledig{{$t['id']}}" onclick="setErledigt({{$t['id']}},{{$log->PPTermineChanges_Id}});"><img src="/data/Icons/fertig.png" style="height:20px;"></button>
                        @endif
                    @endif
