<style>
    ...
    .diff-container:hover .diff-hide {
         display: none;
     }
    .diff-container:hover .diff-show {
        display: block;
    }
    .diff-container {
        border:
...
?>
<div id="printDiff" style='font-family:Arial;'>
    <div>Link zum Produktpass (im Browser): <a style='color black;text-decoration:none;' href='https://tpt-dev.ad.targa.de/show/{{$diffs['pp']['ppid']}}'>{{$diffs['pp']['ian']}}</a></div>
    ...
    <table class='diffArray' style='border-collapse:collapse;'>
        ...
        <?php 
            $d1 = ServiceProvider::printArrayEmail($diff['Wert in Datei 1']);
            $d2 = ServiceProvider::printArrayEmail($diff['Wert in Datei 2']);
            $showLine = true;
