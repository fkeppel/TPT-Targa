   ...
   <div class="table-wrap">
            ...
            <tbody>
                ...
                @forelse($projekts as $projekt)
                    ...
                    <tr class="row-toggle {{ $hasShip ? 'has-shipments' : '' }}"
                      ...
                      <td class="js-log-admin-cell">
    <span class="js-log-admin-text"
    ...
    <select class="js-log-admin"
        ...
        @foreach($data['logMa'] as $maid => $logma)
            <option value="{{ $maid }}"
                {{ isset($projekt->LogAdmin) &&
                   $projekt->LogAdmin == $logma
                    ? 'selected'
                    : '' }}>
                {{ $logma }}
            </option>
                            ...
                            <div class="detail-content" style="background-color: var({{ $bgVar }});">
                                <div style="display:flex; align-items:center; gap:12px;">
                                    ...
                                    <button type="button"
                                            ...
                                            data-master-id="{{ $masterVal }}"
                                            data-archive-url="{{ URL::to('/archivieren') }}">
                                        Archivieren
                                    </button>
                                </div>
    ...
    if (window.__PP_SHIP_SCRIPT_BOUND__) {
        ...
        document.querySelectorAll(".row-toggle").forEach(function (row) {
            row.addEventListener("click", function (e) {
                ...
                if (isHidden) {
                    document.querySelectorAll("tr.row-toggle").forEach(function(otherRow){
        ...
        (function () {
            ...
            var masterRows = document.querySelectorAll('tr.row-toggle');
        ...
        (function () {
            ...
            function getMasterRowFromDetail(detailContent){
                ...
                if (masterRow && masterRow.classList.contains('row-toggle')) {
                    return masterRow;
                }
                return null;
            }
            ...
            document.addEventListener('click', function(e){
                ...
                var tbody = detail ? detail.querySelector('.inner-table tbody') : null;
                if (!detail || !tbody) return;
                // Pro Projekt darf nur eine noch nicht gespeicherte Zeile existieren.
                var unsavedRow = tbody.querySelector('tr.js-shipment-row[data-shipment-id=""]');
                if (unsavedRow) {
                    var existingInput = unsavedRow.querySelector('.js-ship-input');
                    if (existingInput) existingInput.focus();
                    return;
                }
                // Schutz gegen schnellen Doppel- oder Mehrfachklick.
    ...
    document.addEventListener('change', function (e) {
        ...
        xhr.onreadystatechange = function () {
            ...
            if (xhr.status >= 200 && xhr.status < 300 && result.success) {
                ...
                text.style.display = '';
                var row = select.closest('tr.row-toggle');
                if (row) {
                    row.setAttribute(
                        'data-log',
    ...
    document.addEventListener('click', function (e) {
        ...
        xhr.onreadystatechange = function () {
            ...
            if (xhr.status >= 200 && xhr.status < 300) {
                ...
                console.error('Archivierung fehlgeschlagen:', xhr.status, xhr.responseText);
            }
        };
        xhr.send(
            '_token=' + encodeURIComponent('{{ csrf_token() }}') +
            '&PPProduktpass_Id=' + encodeURIComponent(btn.getAttribute('data-master-id'))
        );
    });
</script>