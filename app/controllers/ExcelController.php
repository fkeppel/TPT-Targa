...
class ExcelController extends BaseController {
	...
	private $_lbez = array(	'DE' =>'Deutschland (ohne OnlineShop) (ohne Kaufland)', 
	...
	private function setCellValueAndHight($coord, $text){
		//cpcdebug::cpc_debug($coord, '@RFQ2533');
		// Alle Arten von Zeilenumbrüchen normalisieren (\r\n, \r, \n → \n)
		$normalized = preg_replace('/\r\n|\r|\n/', "\n", $text);
		// Anzahl der Zeilenumbrüche zählen
		$count = substr_count($normalized, "\n");
		// Falls du die tatsächliche Zeilenanzahl willst:
		$lineCountText = explode("\n", $normalized); 
		$lines = count($lineCountText);
		$addLines = 0;
		foreach ($lineCountText as $key => $value){
			if (strlen($value) > 100){
				$addLines ++;
			} 
		}
			...
			if ($mengen){
				foreach($mengen as $menge){
					...
					$laenderMengen[$menge->PPProduktpass_Menge_Country] = 
						array( 'Quantity' => $menge->PPProduktpass_Menge_Quantity, 
								"LT" => $crd, 
	...
	private  function writeExcel_Anfrage($id, $send, $to, $cc, $body, $download, $neu){
		...
		if ($man){
			...
			/* $containers = array('HC' => array('Rot', 'Bar', 'Kop', 'USA'),'40' => array('Rot', 'Bar', 'Kop', 'USA'),'20' => array('Rot', 'Bar', 'Kop', 'USA') );
			$summeContainer = array();
			foreach ($containers as $art => $harbours){
				$summeContainer[$art] = 0;
				foreach ($harbours as $harbour){
					$att = "PPInputManuell_Cont".$art.$harbour;
					$summeContainer[$art] += $man->{$att};
				}
			} */
			//Loading
			//$this->worksheet->setCellValue($this->getCell(3,19),$man->PPInputManuell_Cont20Stk);
			//
			//$this->worksheet->setCellValue($this->getCell(3,20),'XX');
			...
			$this->setCellValueCompare($this->getCell(3,26),$man, $comp, 'PPInputManuell_Masse');
			$this->setCellValueCompare($this->getCell(4,26),$man, $comp, 'PPInputManuell_Laenge');
			$this->setCellValueCompare($this->getCell(5,26),$man, $comp, 'PPInputManuell_Breite');
			$this->setCellValueCompare($this->getCell(6,26),$man, $comp, 'PPInputManuell_Hoehe');
			$row = 27;
