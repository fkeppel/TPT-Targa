...
for ($l = 1; $l <= $sizeCount; $l++) {
    ...
    if ($testOSProLand['OSDE'] > 0) {
        $this->MultiCellSave($w[2], $xcellheight, $m, 'LBTR', 'C');
        $x = $x + $w[2];
    }
    ...
    if ($testOSProLand['OSBE'] > 0) {
        $this->MultiCellSave($w[3], $xcellheight, $m, 'LBTR', 'C');
        $x = $x + $w[3];
    }
    ...
    if ($testOSProLand['OSNL'] > 0) {
        $this->MultiCellSave($w[4], $xcellheight, $m, 'BTLR', 'C');
        $x = $x + $w[4];
    }
    ...
    if ($testOSProLand['OSCZ'] > 0) {
        $this->MultiCellSave($w[5], $xcellheight, $m, 'BTLR', 'C');
        $x = $x + $w[5];
    }
    ...
    if ($testOSProLand['OSES'] > 0) {
        $this->MultiCellSave($w[6], $xcellheight, $m, 'BTLR', 'C');
        $x = $x + $w[6];
    }
    ...
    $m = ($sort['OSMengeGB'] > 0 ) ? number_format($sizeMengen['GB'][$l], 0, ',', '.')
                : "";
    //if (strpos($sort['Laenderblock'],"OSES")!==false  or strlen($sort['Laenderblock'])<1){$this->MultiCellSave($w[6],$xcellheight,$m,'BTLR','C');$x = $x + $w[6];}
    if ($testOSProLand['OSGB'] > 0) {
        $this->MultiCellSave($w[7], $xcellheight, $m, 'BTLR', 'C');
        $x = $x + $w[7];
    }
    ...
    $m = ($sort['OSMengeFR'] > 0 ) ? number_format($sizeMengen['FR'][$l], 0, ',', '.')
                : "";
    //if (strpos($sort['Laenderblock'],"OSES")!==false  or strlen($sort['Laenderblock'])<1){$this->MultiCellSave($w[6],$xcellheight,$m,'BTLR','C');$x = $x + $w[6];}
    if ($testOSProLand['OSFR'] > 0) {
        $this->MultiCellSave($w[8], $xcellheight, $m, 'BTLR', 'C');
    }
    ...
    $xcellheight = $startcellheight;
    $this->fpdf->setXY($startx, $starty);
    $this->MultiCellSave($w[1], $startcellheight, 'XEAN-Code', 'LTBR');
    $xcellheight = $this->fpdf->getY() - $starty;
    $x           = $startx + $w[1];
    $this->fpdf->setXY($x, $starty);
