...
//$this -> MultiCellSave($w[1] + $w[2], 4, 'incOSSortNeu ', 'LTBR');
...
$osm = array('OSDE' => 0, 'OSBE' => 0, 'OSNL' => 0, 'OSCZ' => 0, 'OSES' => 0, 'OSGB' => 0, 'OSFR' => 0);
...
foreach ($sorts as $sort) {
    ...
    If ($sort['hasOSSizeSort']) {
        $OSLaender = array('DE', 'BE', 'NL', 'CZ', 'ES', 'GB', 'FR');
...
foreach ($sortUnique as $sortLB => $sortUAll) {
    ...
    foreach ($sortUAll['sort'] as $sort) {
        ...
        foreach ($OSLaender as $osland) {
            $sOsMenge = 0;
            foreach ($sort['aOSMenge'][$osland] as $mMenge) {
                $sOsMenge += $mMenge;
            }

            if (strpos($sortLB, "OS" . $osland) !== false and $sOsMenge > 0) {
                $aPrint['cts'][$osland] = $sOsMenge;
                $aPrint['qty'][$osland] = $VEs[$osland];
                $aPrint['EAN'][$osland] = $sort['AEAN'];
            }
        }
...
only for OS (Cart./Color sort. solid Type) ' . $sortLB, 'LTBR');
            $this->fpdf->Ln(0);
